...
Goal: To create a useful abstraction for the domain of peer influence on a college campus, to experiment and establish the quantitative effects and boundaries of key parameters, to validate the model against real-world empirical data, and to ascertain the possible effects of certain policies that institutions might choose to enact to encourage racial integration.
\begin{enumerate}
\item College enrollment rates for racial minorities are disproportionately low across the United States.
\item Attrition is consistently higher for minorities than for white students.
\item Alienation has been identified as a contributor to these conlusions.
\end{enumerate}
...
\begin{frame}
\frametitle{Social Phenomenon}
Propinquity: The more often two people encounter one another, the more likely it is they will form a friendship.\\

~~\\

Homophily: People with similar traits interact with one another more often than with dissimilar.
\end{frame}
...
\begin{frame}
\frametitle{Purpose}
CollegeSim is an agent-based model designed to simulate an evolving social network among college students. Student agents are assigned one of two ``races" which factors into their perceived similarity to other agents, and accordingly to their likelihood of forming friendships with them. The ultimate goal of the model is to investigate how racial segregation is affected by the strength of the race trait and the presence or absence of certain institutional policies designed to influence segregation.
\end{frame}
...
\begin{enumerate}
\item At the start of each academic year, Sim runs IncrementYears, AddNewStudents, and AddNewGroups.
\item In each academic month, each Student runs EncounterOthers, AdjustAttributes, and DecayFriendships.
\item In each academic month, each Group runs InfluenceMembers, RecruitStudents, and LoseStudents.
\item At the end of each academic year, Sim runs GraduateStudents.
\end{enumerate}
\end{frame}
